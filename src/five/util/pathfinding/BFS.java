...
public abstract class BFS {
    ...
    public void minerMove(MapLocation target) throws GameActionException {
        minerMove(target, false);
    }
    ...
    void minerMove(MapLocation target, boolean greedy) throws GameActionException {
        if (target == null) return;
        if (!rc.isMovementReady()) return;
        if (rc.getLocation().distanceSquaredTo(target) == 0) return;
    ...
    abstract void initializeBFS();
    public abstract MapLocation miningLocation() throws GameActionException;
}

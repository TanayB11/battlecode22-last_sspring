    ...
    static void runMiner(RobotController rc) throws GameActionException {
        ...
        if (nearbyEnemies.length > 0) {
            ...
            for (int i = 0; i < nearbyEnemies.length; i++) {
                ...
                if (retreatFromEnemy == null && isAttackingEnemy(nearbyEnemies[i])) {
        ...
        //find a mining target
            ...
            //&& its unoccupied && adjacent to lead (more lead wins tiebreaker)
            //if low health, go back for healing
        if (!mineLock && miningTarget == null) {
            ...
            if (nearbyGold.length > 0) {
                ...
                for (int i = 0; i < nearbyGold.length; i++) {
                    for (int j = 0; j < directions.length; j++){
                        if (rc.canSenseLocation(nearbyGold[i].add(directions[j]))) {
                            if (rc.senseRubble(nearbyGold[i].add(directions[j])) < minRubble) {
                                optimalLocation = nearbyGold[i].add(directions[j]);
                                minRubble = rc.senseRubble(nearbyGold[i].add(directions[j]));
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < nearbyLead.length; i++) {
                    for (int j = 0; j < directions.length; j++){
                        if (rc.canSenseLocation(nearbyLead[i].add(directions[j]))) {
                            if (rc.senseRubble(nearbyLead[i].add(directions[j])) < minRubble) {
                                optimalLocation = nearbyLead[i].add(directions[j]);
                                minRubble = rc.senseRubble(nearbyLead[i].add(directions[j]));
                            }
                        }
                    }
                }
            ...
            miningTarget = optimalLocation;
            mineTargetRubble = rc.senseRubble(miningTarget);
        } else if (!mineLock && miningTarget != null) {
            MapLocation[] nearbyGold = rc.senseNearbyLocationsWithGold();
            MapLocation[] nearbyLead = rc.senseNearbyLocationsWithLead(-1, 2);
            ...
            if (rc.senseRubble(optimalLocation) + MINE_TARGET_DIFF_THRESHOLD < mineTargetRubble) {
                miningTarget = optimalLocation;
                mineTargetRubble = rc.senseRubble(optimalLocation);
            }
        }

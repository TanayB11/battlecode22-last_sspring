package voldemort_eight.util;

import battlecode.common.*;
import voldemort_eight.util.data_structures.RingBufferQueue;
...
import static voldemort_eight.util.SafeActions.safeMove;
    ...
    public static MapLocation getBestBuildingLoc(RobotController rc) throws GameActionException {
        ...
        if (rc.senseRubble(me) > ACCEPTABLE_BUILDING_RUBBLE) {
            ...
            for (int i = 0; i < visibleLocs.length; i++) {
                if (
                    rc.senseRubble(visibleLocs[i]) < ACCEPTABLE_BUILDING_RUBBLE && (rc.senseRubble(visibleLocs[i]) < bestRubble) || ((rc.senseRubble(visibleLocs[i]) == bestRubble) &&(me.distanceSquaredTo(visibleLocs[i]) < bestDist))) {
                    bestBuildingLoc = visibleLocs[i];
                    bestRubble = rc.senseRubble(visibleLocs[i]);
